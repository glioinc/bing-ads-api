...
module BingAdsApi
	...
	class ClientProxy
		...
		def build_headers
			headers = {
				"#{NAMESPACE}:CustomerAccountId" => account_id,
				"#{NAMESPACE}:CustomerId" => customer_id,
				"#{NAMESPACE}:DeveloperToken" => developer_token,
			}
			if authentication_token
				headers["#{NAMESPACE}:AuthenticationToken"] = authentication_token
			else
				headers["#{NAMESPACE}:UserName"] = username
				headers["#{NAMESPACE}:Password"] = password
			end
			headers
		end
	end
