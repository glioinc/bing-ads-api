...
module BingAdsApi
  ...
  class Service
    ...
    # * password - Bing Ads user's sign-in password
    # * authentication_token - client application access token (only for OAuth)
    ...
    def initialize(options={})
      ...
      if options[:authentication_token]
        clientProxySettings.merge!(options.slice(:authentication_token))
      else
        clientProxySettings.merge!(options.slice(:username, :password))
      end
    ...
    def solve_wsdl_url
      ...
      config.service_wsdl(environment, get_service_name)
    end
  end
end