...
module BingAdsApi
	...
	class AdGroup < BingAdsApi::DataObject
		...
		def to_hash(keys = :underscore)
			hash = super(keys).except(get_attribute_key("language", keys),
			                          get_attribute_key("ad_rotation", keys))
			...
			if self.search_bid
				hash[get_attribute_key("search_bid", keys)] = {amount_key => self.search_bid}
			end
		...
		private
			def set_custom_attributes(attributes)
				...
				self.search_bid = attributes[:search_bid][:amount] if attributes.key?(:search_bid)
